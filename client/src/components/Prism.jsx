...
const Prism = ({
    ...
    useEffect(() => {
        ...
        // Limit DPR to 1.25 max to drastically improve performance on Retina/4K displays
    ...
    `;
        ...
        const setMat3 = (yY, pX, rZ, out) => {
            const cy = Math.cos(yY), sy = Math.sin(yY), cx = Math.cos(pX), sx = Math.sin(pX), cz = Math.cos(rZ), sz = Math.sin(rZ);
        ...
        const render = t => {
            ...
            if (animationType === 'hover') {
                ...
                if (Math.abs(yaw - targetYaw) < 1e-4 && Math.abs(pitch - targetPitch) < 1e-4 && Math.abs(roll) < 1e-4) go = false;
            } else if (animationType === '3drotate') {
                const ts = time * TS;
            ...
            if (go) raf = requestAnimationFrame(render); else raf = 0;
        };
